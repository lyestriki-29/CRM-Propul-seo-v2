-- Script corrigé pour créer le système de réactions aux messages
...
-- Fonction pour ajouter une réaction (version corrigée)
...
BEGIN
  -- Essayer différentes façons de récupérer l'ID de l'utilisateur actuel
  -- Méthode 1 : chercher par auth.uid() directement
  SELECT id INTO current_user_uuid 
  FROM user_profiles 
  WHERE id::text = auth.uid()::text;
  
  -- Méthode 2 : si la première ne marche pas, chercher par email
  ...
  -- Méthode 3 : dernière tentative, chercher par auth_user_id si elle existe
  IF current_user_uuid IS NULL THEN
    BEGIN
      SELECT id INTO current_user_uuid 
      FROM user_profiles 
    ...
    EXCEPTION WHEN undefined_column THEN
      -- La colonne n'existe pas, on continue
      NULL;
    END;
  END IF;
  
  IF current_user_uuid IS NULL THEN
    RAISE EXCEPTION 'Utilisateur non trouvé dans user_profiles';
  END IF;
...
-- Fonction pour supprimer une réaction (version corrigée)
