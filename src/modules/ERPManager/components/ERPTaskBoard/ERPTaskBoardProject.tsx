...
import type { CommTask, ProjectV2, StatusERP } from '../../../../types/project-v2'
import { ERPTaskCard } from './ERPTaskCard'

type ERPProject = ProjectV2 & { erp_status: StatusERP }

interface ERPTaskBoardProjectProps {
  tasks: CommTask[]
  projects: ERPProject[]
...
export function ERPTaskBoardProject({ tasks, projects, onMoveTask, onClickTask, onAddTask }: ERPTaskBoardProjectProps) {
  ...
  // Projets ERP actifs uniquement (hors perdu/livre)
  const activeProjects = projects.filter(p => !['perdu', 'livre'].includes(p.erp_status))
  ...
  return (
    <DndContext sensors={sensors} onDragStart={handleDragStart} onDragEnd={handleDragEnd}>
      ...
      <DragOverlay>
        {activeTask && <ERPTaskCard task={activeTask} onClick={() => {}} isDragging={false} style={{ rotate: '2deg', boxShadow: '0 16px 40px rgba(0,0,0,0.5)' }} />}
      </DragOverlay>
    </DndContext>
  )
}
