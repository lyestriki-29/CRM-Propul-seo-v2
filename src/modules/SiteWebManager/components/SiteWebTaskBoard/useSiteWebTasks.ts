...
import { MOCK_SITEWEB_TASKS } from '../../mocks/mockSiteWebTasks'
import type { CommTask, CommTaskStatus, CommTaskPriority } from '../../../../types/project-v2'

export type SiteWebTaskView = 'project' | 'month' | 'week'

export interface SiteWebTaskFiltersState {
  priorities: CommTaskPriority[]
  statuses: CommTaskStatus[]
  projectIds: string[]
}

export function useSiteWebTasks(initialView: SiteWebTaskView = 'project') {
  const [tasks, setTasks] = useState<CommTask[]>(MOCK_SITEWEB_TASKS)
  const [view, setView] = useState<SiteWebTaskView>(initialView)

  // Sync si le parent change la vue
  useEffect(() => { setView(initialView) }, [initialView])
  const [filters, setFilters] = useState<SiteWebTaskFiltersState>({
  ...
  const createTask = useCallback((data: Omit<CommTask, 'id' | 'created_at' | 'updated_at'>) => {
    ...
    const task: CommTask = { ...data, id: `swt-${Date.now()}`, created_at: now, updated_at: now }
