...
import type { SiteWebTaskFiltersState, SiteWebTaskView } from './useSiteWebTasks'
import type { CommTaskPriority, CommTaskStatus, ProjectV2, StatusSiteWeb } from '../../../../types/project-v2'
import { PRIORITY_CONFIG, STATUS_CONFIG } from './siteWebTaskConfig'

type SiteWebProject = ProjectV2 & { sw_status: StatusSiteWeb }

interface SiteWebTaskFiltersProps {
  filters: SiteWebTaskFiltersState
  view: SiteWebTaskView
  projects: SiteWebProject[]
  currentDate: Date
  onFiltersChange: (f: SiteWebTaskFiltersState) => void
  onViewChange: (v: SiteWebTaskView) => void
  onPrev: () => void
  onNext: () => void
  onToday: () => void
  onNewTask: () => void
...
export function SiteWebTaskFilters({
  ...
  // Nombre de filtres actifs (non par defaut)
  ...
  return (
    <div className="flex items-center gap-2 px-4 py-2 border-b border-border bg-surface-1/50">
      {/* Navigation periode (mois / semaine seulement) */}
      {view !== 'project' && (
      ...
      <div className="relative ml-2" ref={popRef}>
        ...
        {open && (
          <div className="absolute top-full left-0 mt-1.5 w-72 bg-surface-1 border border-border rounded-xl shadow-xl z-50 p-3 space-y-3">
            {/* Priorite */}
            <div>
              <p className="text-[10px] uppercase tracking-wider text-muted-foreground mb-1.5">Priorite</p>
      ...
      >
        <Plus className="w-3.5 h-3.5" /> Nouvelle tache
      </button>
    </div>
  )
}
