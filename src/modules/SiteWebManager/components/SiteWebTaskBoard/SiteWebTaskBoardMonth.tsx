...
import { SiteWebTaskChip } from './SiteWebTaskChip'

interface SiteWebTaskBoardMonthProps {
  tasks: CommTask[]
  currentDate: Date
...
function DraggableChip({ task, onClickTask }: { task: CommTask; onClickTask: (t: CommTask) => void }) {
  ...
  return (
    <div ref={setNodeRef} style={{ transform: CSS.Translate.toString(transform) }}>
      <SiteWebTaskChip task={task} onClick={onClickTask} isDragging={isDragging} dragHandleProps={{ ...attributes, ...listeners }} />
    </div>
  )
}
...
export function SiteWebTaskBoardMonth({ tasks, currentDate, onMoveTask, onClickTask, onDateClick }: SiteWebTaskBoardMonthProps) {
  ...
  return (
    <DndContext sensors={sensors} onDragStart={handleDragStart} onDragEnd={handleDragEnd}>
      ...
      <DragOverlay>
        {activeTask && <SiteWebTaskChip task={activeTask} onClick={() => {}} style={{ rotate: '3deg', boxShadow: '0 8px 24px rgba(0,0,0,0.5)' }} />}
      </DragOverlay>
    </DndContext>
  )
}
